...
如果你添加了 `"baseUrl": "http://localhost:3000"` 到 `cypress.json` 配置檔案中，則可以直接使用較短的寫法 `cy.visit("/")`，而不用完整寫出 `cy.visit("http://localhost:3000/")`。
...
> **注意：** 或者，你可以安裝 `start-server-and-test` 套件和添加 `"test": "start-server-and-test start http://localhost:3000 cypress"` 到 `package.json` 中，以啟動 Next.js 的生產環境伺服器，並結合 Cypress 使用。請記得在有任何新修改後，重新打包你的應用程式。

### 為持續集成（CI）做好準備

你會注意到，到目前為止執行 Cypress 會自動打開一個瀏覽器，這對 CI 來說不好。如果不想讓他自動打開，你可以用 `cypress run` 來以無頭（headless）模式執行 Cypress：
...
你可以從以下資源中了解更多 Cypress 和 持續集成：

- [Cypress 和 持續集成文件](https://docs.cypress.io/guides/continuous-integration/introduction)
...
最快的入門方法是使用 `create-next-app` 和 [with-playwright 範例](https://github.com/vercel/next.js/tree/canary/examples/with-playwright)。這將創建一個 Next.js 項目，其中包含設定好的 Playwright。
...
你也可以使用 `npm init playwright` 來添加 Playwright 到你現有的 `NPM` 專案。
...
如果你添加了 [`"baseURL": "http://localhost:3000"`](https://playwright.dev/docs/api/class-testoptions#test-options-base-url) 到 `playwright.config.ts` 配置檔案中，則可以直接使用較短的寫法 `page.goto("/")`，而不用完整寫出 `page.goto("http://localhost:3000/")`。
...
> **注意：** 或者你可以使用 [`webServer`](https://playwright.dev/docs/test-advanced#launching-a-development-web-server-during-the-tests) 功能來讓 Playwright 啟動開發服務器並等待它啟動完成。

### 在持續集成（CI）中執行 Playwright
...
你可以從以下資源中了解更多 Playwright 和持續集成：
...
在專案的根目錄中創建一個 `jest.config.js` 檔案，並添加以下內容：
...
- 將 `.env`（和所有變體）加載到 `process.env`
- 從測試解析和轉​​換中忽略 `node_modules`
- 從測試解析中忽略 `.next`
- 加載 `next.config.js`，作為啟用 SWC 轉換的標記

> **注意：** 為了直接測試環境變數，請在單獨的設定腳本或 `jest.config.js` 文件中手動加載它們。請參閱 [測試環境變數](https://nextjs.org/docs/basic-features/environment-variables#test-environment-variables) 章節。

### 設定 Jest（使用 Babel）

如果你在安裝時選擇手動排除 [Rust編譯器](https://nextjs.org/docs/advanced-features/compiler)，除了上述套件外，你還需要手動配置 Jest 並安裝 `babel-jest` 和 `identity-obj-proxy`。
...
測試時並沒有包含樣式表和圖片，因此引入時可能會導致錯誤，因此我們需要模擬它們。在 `__mocks__` 資料夾中創建上面配置引用的模擬文件 - `fileMock.js` 和 `styleMock.js`：
...
`@testing-library/jest-dom` 包括一組方便的 [自定義匹配器](https://github.com/testing-library/jest-dom#custom-matchers)，例如 `.toBeInTheDocument()` 可以幫你確認指定的 DOM 有沒有渲染出來，使寫測試更簡單。你可以添加以下選項到 Jest 配置文件，來為每個測試引入自定義匹配器：
...
如果你需要在每次測試之前添加更多設置選項，通常可以將它們添加到上面的 `jest.setup.js` 文件中。

**可選：絕對引入和模組路徑別名**

如果你的專案中使用 [模組路徑別名](https://nextjs.org/docs/advanced-features/module-path-aliases)，你需要配置 Jest 來匹配 `jsconfig.json` 文件中的路徑選項與 `jest.config.js` 文件中的 `moduleNameMapper` 選項，以解析導入。例如：
...
將監視模式下的 Jest 可執行文件添加到 `package.json` 腳本中：
...
你的專案現在已準備好執行測試。根據 Jests 的慣例，將測試添加到專案根目錄中的 `__tests__` 資料夾。

例如，我們可以添加一個測試來檢查 `<Home />` 元件是否成功呈現標題：
...
或者，添加 [快照測試](https://jestjs.io/docs/snapshot-testing) 以追蹤 `<Home />` 元件有沒有任何的非預期變更：
...
輸入 `npm run test` 來執行你的測試。在你的測試通過或失敗後，你會注意到一個互動式 Jest 指令列表，在你要添加更多測試時，這些指令會很有幫助。
