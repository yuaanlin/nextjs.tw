...
`next/image` 元件和 Next.js 圖片優化 API 可於 [`next.config.js`](/docs/api-reference/next.config.js/introduction.md) 檔案做設定，您可於此檔案 [處理遠端圖片](/docs/api-reference/next/image.md#domains)、[客製化響應式設計的圖片分界點](/docs/api-reference/next/image.md#device-sizes)、[調整緩存機制](/docs/api-reference/next/image.md#caching-behavior) 或更多相關設定。
