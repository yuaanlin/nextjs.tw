...
[`getServerSideProps` API 文件](/docs/api-reference/data-fetching/get-server-side-props) 涵蓋所有可以使用的參數和props。
...
當您想從服務器獲取數據時，可能很容易使用 [API 路由](/docs/api-routes/introduction)，然後從 `getServerSideProps` 調用該 API 路由。 這是一種不必要且低效的方法，因為它會由於服務器上運行的 `getServerSideProps` 和 API Routes 而導致發出額外的請求。
...
例如，這種方法適用於用戶儀表板頁面。 因為儀表板是一個私人的、用戶特定的頁面，所以這頁不需要做SEO ，並且該頁面不需要預渲染。 數據經常更新，這需要在請求時獲取數據。
## 使用 getServerSideProps 在請求時獲取數據

以下範例顯示如何在請求時獲取數據並預渲染結果。
...
如果在執行`getServerSideProps`發生錯誤，他將會顯示`pages/500.js`頁面。 查看此文件[500 page](/docs/advanced-features/custom-error-page#500-page) 了解有關如何創建它的更多信息.在開發過程中，不會使用此文件 
並且將顯示dev overlay（不會翻）。
