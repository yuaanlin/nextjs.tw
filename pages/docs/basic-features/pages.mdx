...
> **注意：** 我們正在將改進過後的新版路由引入 Next.js。閱讀 [Layouts RFC](https://nextjs.org/blog/layouts-rfc) 以獲得更多詳細資訊並提供回饋。

在 Next.js 中，一個 **頁面 (Pages)** 是一個從 `pages` 資料夾底下的 `.js`、`.jsx`、`.ts` 或 `.tsx` 檔案匯出的 [React 元件](https://reactjs.org/docs/components-and-props.html)。每個頁面都會根據其檔案名稱與路由相關聯。

**範例**：如果你建立了 `pages/about.js` 檔案，並匯出了如下的 React 元件，你將會在網站的 `/about` 頁面下看到他。
...
在預設的情況下，Next.js 會 **提前渲染 (pre-render)** 每個頁面。這意味著 Next.js 會提前為每個頁面產生 HTML，而不是讓所有事情都由客戶端 JavaScript 完成，這可以為我們帶來更好的效能與 SEO。

每個產生的 HTML 都會與該頁面所需要的最少 JavaScript 代碼相關聯。當瀏覽器載入一個頁面時，它的 JavaScript 程式碼會執行並使頁面可以讓使用者進行互動。 (這個過程被稱為 _hydration_。)

### 兩種形式的提前渲染

Next.js 有兩種形式的提前渲染：**靜態生成 (Static Generation)** 和 **伺服器端渲染 (Server-side Rendering)**。兩者的差別在於它們產生頁面 HTML 的**時機**。

- [**靜態生成 (推薦)**](#靜態生成-推薦)：HTML 會在 **建置時期 (build time)** 產生，並且會在每個請求中重複使用。
...
注意到這個頁面不需要抓取任何外部資料來預先渲染。在這種情況下，Next.js 在建置 (build time) 時為每個頁面產生一個 HTML 檔案。
...
為了在預先渲染時抓取這些資料，Next.js 允許你從同一個檔案 `export` 一個 `async` 函數，這個函數叫做 `getStaticProps`。這個函數會在建置 (build time) 時被呼叫，並讓你在預先渲染時傳遞抓取到的資料給頁面的 `props`。
...
然而，你在建置 (build time) 時想要預先渲染哪個 `id` 可能會取決於外部資料。

**範例**：假設你只在資料庫中新增了一篇部落格文章 (並且他的 `id: 1`)。在這種情況下，你只想在建置 (build time) 時預先渲染 `posts/1`。

後來，你可能會新增第二篇部落格文章 (並且他的 `id: 2`)。然後你也想要預先渲染 `posts/2`。

所以你的頁面 **路徑** 會依賴外部資料。為了處理這個情況，Next.js 允許你從一個動態頁面 (在這個範例中，指的是 `pages/posts/[id].js`) `export` 一個 `async` 函數，叫做 `getStaticPaths`。這個函數會在建置 (build time) 時被呼叫，並讓你指定哪些路徑你想要預先渲染。
...
為了使用伺服器端渲染，你需要 `export` 一個 `async` 函式，並且命名為 `getServerSideProps`。這個函式會在每個請求中被伺服器呼叫。
...
如同上面你所看到的，`getServerSideProps` 與 `getStaticProps` 很相似，但是它們的差別在於 `getServerSideProps` 會在每個請求中被呼叫，而不是在建構時。
...
- **靜態生成（推薦）：** HTML 會在 **建構時** 被生成，並且會在每個請求中被重複使用。要讓頁面使用靜態生成，你可以 `export` 頁面元件，或者 `export` `getStaticProps`（如果需要的話也可以 `export` `getStaticPaths`）。這對於可以在使用者請求之前預先渲染的頁面來說是很棒的。你也可以和客戶端渲染一起使用，以便帶入額外的資料。
