...
檢查用戶提供的資訊是否正確的過程。 表單驗證還確保提供的資訊格式是正確的（例如，電子郵件欄位中有一個 @）。
表單驗證有兩種類型：
...
有了這些驗證檢查，當用戶嘗試在 Name 欄位為空時提交表單，會在表單中的該欄位位置彈出錯誤。同樣地，只有 10-20 個字元長度的值才能輸入到正式編號（Roll Number）欄位。
...
表單驗證對於確保用戶以正確的格式提交正確的資料很重要。JavaScript 在客戶端提供了額外級別的驗證以及 HTML 原生表單屬性。開發人員通常更喜歡通過 JavaScript 驗證表單資料，因為與伺服器端驗證相比，它的資料處理速度更快，但是在某些情境下單靠前端驗證可能不太安全，因為惡意用戶總是有辦法向您的伺服器發送格式錯誤的資料。
...
本例子驗證用戶的姓名和正式編號。`validateFormWithJS()` 函數不允許為空，並且正式編號必須至少為三位數。當您點擊提交按鈕時即進行驗證。唯有欄位的值被正確填入後提交，您才會被重定向到下一頁。
...
欲建立一個新的 Next.js 應用程式，您可以使用 [create-next-app](https://nextjs.org/docs/getting-started#setup) 快速建立。在命令行終端機中，執行以下命令：
...
Next.js 提供了一個基於文件的路由系統，該系統以 [頁面概念](/docs/basic-features/pages) 為基礎建立而成。`pages/api` 文件夾中的所有文件都會映射到 `/api/*`，並將被視為 API 端點而不是頁面。這個 [API 端點](/docs/api-routes/introduction) 將僅在伺服器端。

在 `pages/api` 路由下，創建一個名為 `form.js` 的文件，打開後貼上這段用 Node.js 編寫的代碼：

```js
export default function handler(req, res) {
  // 獲取在 request 的 body 中被用戶提交的資料
  const body = req.body;
...
> 此外，您還可以將此 API 附加 MongoDB 或 Google Sheets 等資料庫。這樣，您提交的表單資料將被安全儲存以便之後使用。在本指南，並未使用資料庫，而是將相同的資料回傳給用戶以演示它是如何完成的。
...
> 您可以查看我們在此處作為範例建立的 [next-forms](https://github.com/vercel/next.js/tree/canary/examples/next-forms) 範例儲存庫完整源代碼。歡迎複製它並即刻開始嘗試。本例子使用 create-next-app 建立，您可以在 `/styles/global.css` 文件中預覽基本的表單 CSS 樣式。
...
接下來打開 `next-forms` 資料夾。在 `/pages` 資料夾中，建立一個文件 `no-js-form.js`。

> **快速提示**：在 Next.js 中，頁面是從 pages 目錄中的 `.js`、`.jsx`、`.ts` 或 `.tsx` 文件導出的 React 元件。每個頁面都會根據其文件名配置一個路由。
>
...
> 為了改善體驗，您可以回傳一個響應，將用戶重定向到一個表示感謝他們提交表單的頁面。

## Part 6: 啟用 JavaScript 的表單提交

在 `/pages` 中，您將建立另一個名為 `js-form.js` 的文件。這將在您的 Next.js 應用程式上創建一個 `/js-form` 頁面。

現在，一旦表單被提交，我們會阻止表單預設的重新加載頁面行為。當我們獲取表單資料，會將其轉換為 JSON 字符串，然後發送到我們的伺服器，即 API 端點。最後，我們的伺服器將響應用戶提交的姓名。所有這一切都透過一個基本的 JavaScript `handleSubmit()` 函式實現。
