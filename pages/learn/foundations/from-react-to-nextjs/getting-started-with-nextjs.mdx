...
當安裝程序完成後，你可以在 `package.json` 文檔中，查看專案所有的相依套件與應用程式資訊：
...
當你儲存調整後的程式碼，你應該會發現瀏覽器已自動重新渲染頁面，以反映你在程式碼上的更動，這樣的特色稱為[快速重新整理( Fast Fresh )](https://nextjs.org/docs/basic-features/fast-refresh)。它會對你在編輯程式碼後提供即時性的回饋，並讓 Next.js 預先針對這些回饋進行配置。
簡單回顧一下，你的程式碼從先前的結構...
...
看完前面對於程式碼上的變更，你或許已經理解使用 Next.js 的好處。你移除了 Babel Script， 體驗了不再需要多餘思考而配置的複雜工具。你也觀察到了[快速重新整理]( Fast Refresh )的實際效果，而這些只是 Next.js 在眾多開發者體驗特色中的其中一項。
