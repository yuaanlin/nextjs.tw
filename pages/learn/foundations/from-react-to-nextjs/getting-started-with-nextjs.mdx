...
在準備開發前，請先新增一個內容為空物件 `{}` 的 `package.json` 文檔。
...
您可透過終端機運行 `npm run dev` 並開啟瀏覽器前往 [localhost:3000](localhost:3000) 以查看應用程式是否正常運作。接下來，您可以針對程式碼試著做些調整並儲存

當您儲存調整後的程式碼，您應該會發現瀏覽器已自動重新渲染頁面，以反映您在程式碼上的更動，這樣的特色稱為[快速刷新( Fast Fresh )](https://nextjs.org/docs/basic-features/fast-refresh)。它會對您在編輯程式碼後提供即時性的回饋，並讓 Next.js 預先針對這些回饋進行配置。
...
從表面上來看，這些過程只是減少了幾行程式碼，但它確實幫助凸顯了一些事情：React 是一種**函式庫**，它提供了構築現代互動式 UI 的**一切基本**事物，但仍需要透過一些工序將您製作的 UI 整合至應用程式中。

看完前面對於程式碼上的變更，您或許已經理解使用 Next.js 的好處。您移除了 Babel Script， 體驗了不再需要多餘思考而配置的複雜工具。您也觀察到了[快速刷新]( Fast Refresh )的實際效果，而這些只是 Next.js 在眾多開發者體驗特色中的其中一項。
