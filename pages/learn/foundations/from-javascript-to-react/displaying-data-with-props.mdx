...
到目前為止，如果你要重複使用 `<Header />` 組件，它每次均會顯示相同的內容。
...
同樣，你可以將訊息作為屬性傳遞給 React 組件。這些被稱為 `props（屬性）`。

![](https://nextjs.org/static/images/learn/foundations/props.png)

與 JavaScript 函數類似，你可以設計接受自定義參數（或屬性）的組件，這些參數（或屬性）會改變組件的行為或渲染到畫面時可見的內容。然後，你可以將這些屬性從父組件傳遞給子組件。

> **注意:** 在 React 中，資料沿著組件樹向下流動。這被稱為 *單向資料流*。 我們將在下一章節中討論 State（狀態），它可以作為屬性從父組件傳遞到子組件。

### 使用屬性

在 `HomePage` 組件中，你可以將自定義 `title` 屬性傳遞給 `Header` 組件，就像傳遞 HTML 屬性一樣：
...
而子組件 `Header` 可以接受這些屬性作為其第一個 **函數參數**：
...
你的組件現在接受一個通用的 title 屬性，你可以在應用程式的不同部分重複使用它，你需要做的就是更改 title：
...
- [將屬性傳遞給組件](https://beta.reactjs.org/learn/passing-props-to-a-component)
