...
React 有一組稱為 [鉤子](https://reactjs.org/docs/hooks-intro.html) 的函數。鉤子讓你可以向元件添加額外的邏輯，例如 **狀態**。你可以將狀態視為 UI 中隨時間變化的任何訊息，通常由用戶交互觸發。
...
{/*
    ...
    quiz:{
        ...
        options:[
            {
                text: 屬性是傳遞給元件的唯讀訊息；狀態是可以隨時間變化的訊息，通常由用戶交互觸發,
                isCorrect: true
            },
            {
...
- [狀態：元件​​的記憶](https://beta.reactjs.org/learn/state-a-components-memory#)
